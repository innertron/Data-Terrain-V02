...
When the user references cells as `X#X#` / `X#Z#` for spot fixes, the SECOND number is the stored grid row (1-based, row 25 = high-income end): edit `grid[n2 - 1][x - 1]`. It is NOT the display Z. (Elizabeth Vargas fix ping-ponged twice before this was confirmed.)
**How to apply:** print target values first and confirm they are the non-zero ones the user wants removed before writing.
