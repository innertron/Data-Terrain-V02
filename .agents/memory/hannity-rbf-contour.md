...
**NEW preferred input format (Aug 10 2026, user-supplied cell traces):** user now provides contour lines as exact cell lists ("0.54: X1Z23, X2Z23, ..."). Do NOT feed these into exact interpolation (dense cell-snapped points ring — 17 spurious maxima). Use `generateGridFromTraces` in scripts/generate-layer-grid.cjs: least-squares multiquadric, C=4, ridge=1e-3, basis centers every 3 cells, fitted to ALL trace cells. Cells claimed by two contours get the average. Still add corner anchors + sparse fill points midway between widely-separated contours (worked example: data/hannity-trace-points.json). Charts CAN have legitimate troughs/ridges (this one has a valley at X19-20) — validate against chart topology, not blanket monotonicity.
